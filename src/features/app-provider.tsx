...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const saveAttempt = async (a: Attempt) => {
    ...
    if (a.reviewOf) { const old = await storage.readOne<MistakeRecord>("mistakes", a.reviewOf); if (old) records.push({ store: "mistakes", value: { ...old, resolved: a.correct, revisitCount: old.revisitCount + 1, nextReview: new Date(Date.now() + [1, 3, 7, 21][Math.min(old.revisitCount + 1, 3)] * 86400000).toISOString() } }); }
